 ...
 * to 60, so one tip either way flips a match — and the two bots take their elements from the SAME
 * 40, so the stronger one's advantage is capped by what it can physically reach before the other
 * does. Measured at the tuning that ships: HARD scores 113 mean against an idle opponent where
 * EASY scores 54 (2.1x), and that 2.1x converts to a 65% head-to-head win rate.
 *
 ...
 */
const BB_AI_WIN_RATE_FLOOR = 0.6;
