...
# physically. Decimation is FARTHEST-POINT SAMPLING (deterministic: it always starts from the
# lexicographically smallest point), then the residual — the largest distance from any dropped
...
def hull_of(points, name: str, residuals: list[tuple[str, float]]):
    ...
    if len(verts) > HULL_MAX_VERTS:
        ...
        verts = kept
    return [round(float(c), 4) for v in verts for c in v]
...
def main() -> None:
    ...
    for side, pos in wall_buckets.items():
        if not pos:
            continue
        pts = hull_of(pos, f"wall_{side}", residuals)
        if pts:
            statics.append({"name": f"wall_{side}", "class": "wall", "points": pts})
