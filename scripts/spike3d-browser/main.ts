// Browser copy of scripts/spike3d.ts's runSpike, for the Day 0 cross-runtime hash check
// (docs/biobuzz/plan-3d.md #10, #3.9). Deliberately duplicated rather than imported: importing
// the Node CLI file would drag `process`/CLI-parsing references into a Vite browser bundle for
// no benefit, and this whole directory is throwaway. Keep it in sync with spike3d.ts by eye —
// it is not built or tested by anything else in the repo.
//
// Served with `npx vite scripts/spike3d-browser --port 5199`; writes the summary JSON to
// document.body and console.log so a headless Electron runner (or a person) can read it.
...
async function runSpike(opts: SpikeOpts): Promise<SpikeSummary> {
  ...
  addStatic(200, 200, 10, 0, 0, -10);
  addStatic(10, 82, 20, 72, 0, 20);
  addStatic(10, 82, 20, -72, 0, 20);
  addStatic(82, 10, 20, 0, 72, 20);
  addStatic(82, 10, 20, 0, -72, 20);
  addStatic(0.5, 19.4, 0.5, 24.5, 0, 0.5);
  addStatic(0.5, 19.4, 0.5, -24.5, 0, 0.5);
  ...
  for (const [x, y] of robotPositions) {
    const desc = RAPIER.RigidBodyDesc.newDynamic()
      ...
      .setAngularDamping(2);
    const body = world.createRigidBody(desc);
    body.setAdditionalMass(30, true);
...
runSpike({ ticks: 3600, seed: 1 })
  .then((summary) => {
    const line = JSON.stringify(summary);
    out.textContent = line;
    console.log(line);
    (window as any).__spikeSummary = summary;
  })
  .catch((err) => {
    const msg = 'SPIKE_ERROR: ' + (err && err.stack ? err.stack : String(err));
    out.textContent = msg;
    console.error(msg);
  });
