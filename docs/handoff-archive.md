...
Sessions older than the ones in `HANDOFF.md`, in the same reverse-chronological order,
moved here on 2026-09-16. **Nothing was edited or dropped** — this is a cut of the log,
not a summary of it.

Why: `HANDOFF.md` is read at the start of every session, and it had grown to 5,888 lines
(~97,000 tokens). That is a large fixed cost to pay before any work starts, and almost all
of it is history — the standing rules that outlived a session were promoted into
`CLAUDE.md` long ago, which is where they belong and where they are actually loaded.

So `HANDOFF.md` now carries the recent sessions and this file carries the rest. Search
here the same way you would have searched there; `git log -- HANDOFF.md` still has every
version of the original file if you need one.
