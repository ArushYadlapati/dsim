...
3. `biobuzzResultsRows`: every Table 10-2 row from `BbAllianceScore` as alliance-relative `[label, mine, opp]` — LEAVE, PARK (auto), PARK (teleop), TIPS, CELL contents, OWNED FLOWER elements, BOTTOM NECTAR bonus, GARDEN, FOULS, TOTAL. Counts and points both where the row has both. (Three RP flags closed this table until 2026-09-21; see BIOBUZZ-AUDIT BB-11.)
...
**Round 6 closed at `alpha` `8b96dd7`** — A6a (the human-player NECTAR button on bit 128, G304 start legality, the FLOWER section) merged, plus four shared-core landings from the master chat (`GameSimModule.startLegal`, `PinSolid`, `ControlGeometry`, per-artifact radius), the `nectarWhy` chip, a real replay round-trip for bit 128, and the APPROX ledger in `docs/biobuzz/feedback/002-thresholds.md`. ⚠️ Two `server/room.ts` changes ride on it (start-legality dispatch, participation credit) and are INERT until `./scripts/fly-deploy.sh` runs. Three owner questions carry into Round 7: yellow cards game-wide, the spill's short tail, the G304 frontage.
