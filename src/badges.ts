/**
 * BADGES — the rarer half of the rewards ledger, and the half that COUNTS.
 *
 * A TITLE is one-off: it names one thing that happened ("BIOBUZZ · Act 2 · 1v1 Champion") and
 * earning a different one gives you a second title. A BADGE is a CATEGORY of achievement with a
 * counter on it, the way a mastery badge carries a level: earning the same badge again raises
 * its count rather than adding a row, and the badge beside a name shows the number (owner,
 * 2026-09-22: "badges should not be handed out as often as titles. Often, badges should have a
 * counter").
 *
 * ── A LEAF, ON PURPOSE ──────────────────────────────────────────────────────────────────────
 ...
 * `items`, so it is a stable KEY — never a label. Four exist:
...
export const BADGE_KEYS = ['ranked-gold', 'ranked-silver', 'ranked-bronze', 'record-holder'] as const;
 ...
 *  plainer. */
export type BadgeTier = 'gold' | 'silver' | 'bronze' | 'record';
 ...
 * row stays a name with some decoration rather than decoration with a name in it. The title
 * stays one — it is words, and two sentences beside a name do not fit anywhere a name appears.
 */
