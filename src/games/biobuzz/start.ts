...
function extents(spec: RobotSpec, headingDeg: number): { ax: number; ay: number; ox: number; oy: number } {
  ...
  // `dcos`/`dsin`, not the engine's — this file is sim code (`elements.ts` `evalStart` calls it,
  // and the spawner seats a pose through it), so it is bound by the determinism rule the first
